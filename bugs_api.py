...
class BugsApi:
    def __init__(self):
        # client id from the Bugs android app
        self.device_id = None
    ...
    def _get(self, endpoint: str, sub: str = 'mapi', params: dict = None):
        ...
        r = self.s.get(f'https://{sub}.bugs.co.kr/music/5/{endpoint}', params=params, headers=self.headers())
    ...
    def _post(self, endpoint: str, sub: str = 'mapi', params: dict = None, json=None, additional_headers=None):
        # function for API requests
        if not params:
            params = {}
        ...
        r = self.s.post(f'https://{sub}.bugs.co.kr/music/5/{endpoint}', params=params, json=json, headers=headers)
    ...
    def get_artist(self, artist_id: str or int):
        ...
        return self._post('multi/invoke/map', json=[{
            "id": "artist",
            "args": {
                "artist_id": artist_id,
                "result_type": "DETAIL"
    ...
    def get_lyrics(self, track_id: str or int):
        return self._get(f'track/{track_id}/lyrics')

    def get_stream(self, track_id: int, bitrate: str = 'flac'):
        # bitrate is either 'flac24', 'flac', 'aac256', 'aac', '320k'
        return self._get(f'play/track/{track_id}/streaming', params={
            'bitrate': bitrate,
            'wwan': 'N',
            'overwrite_session': 'Y'
        }).get('result')
    ...
    def get_search_individually(self, query: str, category: str = 'track', page: int = 1, limit: int = 100):
        return self._get(f'search/{category}', params={
            'query': query,
            'page': page,
            'size': limit,
            'sort': 'exact',
